    ...
    <a href="AdminDashboard.php" class="inline-flex items-center gap-2 text-indigo-600 hover:text-indigo-800 font-semibold transition-colors text-sm text-decoration-none">
        <i class="fa-solid fa-arrow-left"></i> Back to Dashboard
            ...
            <div class="mb-8">
                ...
                <p class="text-gray-500 mt-1">Inactive disabled accounts (<?= $disabled_count ?>)</p>
            </div>

            <div class="flex flex-col gap-4">
                
                <?php if (empty($disabled_users)): ?>
                ...
                <?php else: ?>
                    <?php foreach ($disabled_users as $user): ?>
                        <a href="AdDisabledDetail.php?id=<?= h($user['id']) ?>" class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center justify-between cursor-pointer hover:shadow-md hover:border-red-200 transition-all duration-200 group block">
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <h4 class="font-bold text-gray-900 group-hover:text-red-600 transition-colors">
                                        <?= h($user['full_name']) ?>
                                    </h4>
