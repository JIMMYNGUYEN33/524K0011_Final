...
// 4. Lấy danh sách tài khoản bị khóa vĩnh viễn (is_permanently_locked = 1)
try {
    $stmt = $pdo->query(
        "SELECT id, full_name, email, phone, balance, status, permanently_locked_at 
         FROM Users 
         WHERE is_permanently_locked = 1 
...
$locked_count = count($locked_users);
?>
<!DOCTYPE html>
<html lang="vi">
<head>
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <main class="flex-1 p-8 overflow-y-auto">
            ...
            <div class="mb-8">
                ...
                <p class="text-gray-500 mt-1">Accounts temporarily or permanently locked (<?= $locked_count ?>)</p>
            </div>
            ...
            <div class="flex flex-col gap-4">
                ...
                <?php if (empty($locked_users)): ?>
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center text-gray-500">
                        <i class="fa-solid fa-user-shield text-4xl mb-3 text-gray-300"></i>
                        <p>No locked accounts found.</p>
                    </div>
                <?php else: ?>
                    <?php foreach ($locked_users as $user): ?>
                        <a href="AdLockedDetail.php?id=<?= h($user['id']) ?>" class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center justify-between cursor-pointer hover:shadow-md hover:border-orange-200 transition-all duration-200 group block">
